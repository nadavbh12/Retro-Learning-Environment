...
FIFOController::FIFOController(AleSystem* _alesystem, bool named_pipes) :
  ALEController(_alesystem), m_named_pipes(named_pipes) {
  m_max_num_frames = _alesystem->settings().getInt("max_num_frames");
  m_run_length_encoding = _alesystem->settings().getBool("run_length_encoding");
}
